...
public class InMemoryIndexRepository implements IndexRepository {
  ...
  /**
   * addition: real (in-memory) slice/dice resolution. Each dimension's hierarchy in {@code
   * sliceDice} must be a prefix of the stored context's hierarchy on the same dimension;
   * unconstrained dimensions match anything. Empty {@code sliceDice} returns every stored context.
   */
  @Override
  ...
  /**
   * addition: stored contexts whose own hierarchies are <em>prefixes</em> of the slice/dice
   * (i.e. they would roll up to the query). Returns only the context ids per {@link
  ...
  // Helper to create Map.Entry instances without AbstractMap.SimpleImmutableEntry verbosity at call
  // sites
