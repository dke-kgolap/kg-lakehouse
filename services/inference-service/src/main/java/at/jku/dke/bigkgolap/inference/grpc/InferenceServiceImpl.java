...
public class InferenceServiceImpl extends InferenceServiceGrpc.InferenceServiceImplBase {
  ...
  public void infer(InferRequest request, StreamObserver<InferResponse> observer) {
    ...
    try {
      ...
      stream(result.derivedTriples(), result.fromCache(), observer);
  ...
  private void stream(
      List<String> triples, boolean fromCache, StreamObserver<InferResponse> observer) {
    InferResponse.Builder builder = InferResponse.newBuilder();
    int inChunk = 0;
    for (String triple : triples) {
      builder.addDerivedTriples(triple);
      if (++inChunk >= chunkTarget) {
        observer.onNext(builder.build());
        builder.clearDerivedTriples();
        inChunk = 0;
      }
    }
